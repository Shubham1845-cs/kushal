...
- **REQUIREMENTS**: Follow kluster_code_review_auto field descriptions strictly. For "modified_files_path":
  - **NEVER pass just a filename** — always a full path.
  - **Get the workspace path first**: run `pwd` (macOS/Linux) or `cd` (Windows) once, then prepend it to the filename. Trust the output — don't run additional commands to cross-check it.
  - **macOS/Linux**: absolute (`/Users/.../project/file.cpp`) OR home-relative with full path from home (`~/projects/myproject/file.cpp`). Never `~/file.cpp` alone.
  - **Windows**: always absolute with drive letter (`C:\Users\...\file.cpp`). `~/` does NOT work.
  - **Multiple files**: separate with `;` (e.g., `/path/a.cpp;/path/b.cpp`).
...
- **SPECS MODE**: Do not call kluster_code_review_auto for planning OR spec mode, BUT include requirement to call kluster in final specs - like last task in tasks file.
